...
export const Input = React.forwardRef(function Input({ label, className, ...props }: InputProps, ref: LegacyRef<HTMLInputElement> | undefined) {
  return (
    <label className="w-full lg:max-w-md">
      ...
      <input ref={ref} {...props} className={
        clsx(
          "mt-1 w-full text-text text-lg font-semibold bg-background py-4 px-2 rounded-lg border border-secondaryAccent dark:border-darkSecondaryAccent",
          className
        )
      }/>
    </label>
...
export function SearchInput({ defaultValue, ...props }: SearchInputProps) {
  const [debouncedValue, setValue] = useDebounce(1000, defaultValue);
...
export function Attachment({ label, attachmentUrl, disabled = false, download = true }: AttachmentProps) {
  return (
